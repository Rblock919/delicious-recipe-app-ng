    ...
    if (slices[slices.length - 1] !== 'edit') {
      if (this.sessionService.isAuthenticated) {
        ...
        this.loggerService.consoleLog(
          'user is not auth, forwarding to login page and saving url'
        );
  ...
  public canDeactivate(
    ...
    if (context === 'editRecipe') {
      ...
      if (tempComponent.recipeForm.dirty && !tempComponent.submitted) {
        const recipeName =
          tempComponent.recipeForm.get('title').value || 'New Recipe';
      ...
      if (
        tempComponent.submitted === false &&
        (tempComponent.userInfo.username !== '' ||
          tempComponent.userInfo.password !== '' ||
          tempComponent.confirmPassword)
      ) {
